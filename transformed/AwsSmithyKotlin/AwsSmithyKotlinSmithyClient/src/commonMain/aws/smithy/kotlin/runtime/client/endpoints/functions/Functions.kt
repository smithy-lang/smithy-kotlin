...
@InternalApi
public fun substring(value: String?, start: Int, stop: Int, reverse: Boolean): String? = value?.let {
    when {
...
@InternalApi
public fun isValidHostLabel(value: String?, allowSubdomains: Boolean): Boolean = value?.let {
...
@InternalApi
public fun parseUrl(value: String?): Url? = value?.let {
    val sdkUrl: SdkUrl
    try {

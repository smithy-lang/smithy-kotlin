...
class AliasConfigurationDeserializer2: HttpDeserialize {
    ...
    override suspend fun deserialize(response: HttpResponse, provider: DeserializationProvider): AliasConfiguration2 {
        // FIXME - what exception do we want here?
        val payload = response.body.readAll() ?: throw ClientException("expected a response payload deserializing AliasConfiguration2")
        val deserializer = provider(payload)
        val builder = AliasConfiguration2.dslBuilder()
        ...
        deserializer.deserializeStruct(OBJ_DESCRIPTOR) {
            loop@while(true) {
                when(findNextFieldIndex()) {
                    ALIAS_ARN_FIELD_DESCRIPTOR.index -> AliasArnTypeDeserializer.deserialize(deserializer)

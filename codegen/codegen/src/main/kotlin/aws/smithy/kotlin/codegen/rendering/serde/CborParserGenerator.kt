...
class CborParserGenerator(
    ...
    override fun operationDeserializer(
        ...
        return op.bodyDeserializer(ctx.settings) { writer ->
            ...
            // across every deserialize call, rather than being rebuilt on each invocation.
    ...
    private fun documentDeserializer(
        ...
        return shape.documentDeserializer(ctx.settings, symbol, members) { writer ->
            // Hoist the descriptor block to file/top-level scope so it is constructed once per class-load and reused
            // across every deserialize call, rather than being rebuilt on each invocation. render() is a no-op for
            // document shapes (no members), so it is safe to invoke unconditionally here.

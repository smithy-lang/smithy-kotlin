...
internal class DefaultCanonicalizer(private val sha256Supplier: HashSupplier = ::Sha256) : Canonicalizer {
    ...
    override suspend fun canonicalRequest(
        ...
        val hash = when (config.bodyHash) {
            is BodyHash.CalculateFromPayload -> request.body.calculateHash()
            else -> config.bodyHash.hash!!
        }
        ...
        val headers = builder
            ...
            .associateTo(mutableMapOf()) { it.key.lowercase() to it.value }
            .entries
            .sortedBy { it.key }
        val signedHeaders = headers.joinToString(separator = ";") { it.key }
...
private fun Map.Entry<String, List<String>>.canonicalLine(): String {
    val valuesString = value.joinToString(separator = ",") { it.trimAll() }

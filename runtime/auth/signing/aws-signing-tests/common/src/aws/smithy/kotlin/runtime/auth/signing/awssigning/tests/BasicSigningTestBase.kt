...
private val CHUNKED_TEST_CREDENTIALS_PROVIDER = StaticCredentialsProvider(Credentials(CHUNKED_ACCESS_KEY_ID, CHUNKED_SECRET_ACCESS_KEY))
...
abstract class BasicSigningTestBase : HasSigner {
    ...
    fun testSignRequestSigV4() = runTest {
        ...
        assertEquals(expectedDate, result.signedRequest!!.headers["X-Amz-Date"])
        assertEquals(expectedAuth, result.signedRequest!!.headers["Authorization"])
    ...
    fun testSignRequestSigV4Asymmetric() = runTest {
        ...
        val authHeader = result.signedRequest!!.headers["Authorization"]!!
    ...
    private fun createChunkedRequestSigningConfig(): AwsSigningConfig = AwsSigningConfig {
        ...
        bodyHashSource = BodyHashSource.StreamingAws4HmacSha256Payload
    ...
    fun testSignChunks() = runTest {
        ...
        assertEquals(EXPECTED_CHUNK_REQUEST_AUTHORIZATION_HEADER, requestResult.signedRequest!!.headers["Authorization"])

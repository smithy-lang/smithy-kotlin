...
public class CrtHttpEngine(public val config: CrtHttpEngineConfig) : HttpClientEngineBase("crt") {
    ...
    private val resolvedTlsContext: TlsContext = config.tlsContext ?: run {
        val options = TlsContextOptionsBuilder().apply {
            verifyPeer = true
            alpn = config.alpn.joinToString(separator = ";") { it.protocolId }
            minTlsVersion = toCrtTlsVersion(config.minTlsVersion)
        }.build()
        TlsContext(options)
    }
...
private fun toCrtTlsVersion(sdkTlsVersion: SdkTlsVersion): CrtTlsVersion = when (sdkTlsVersion) {
    SdkTlsVersion.Tls1_0 -> CrtTlsVersion.TLSv1
    SdkTlsVersion.Tls1_1 -> CrtTlsVersion.TLS_V1_1
    SdkTlsVersion.Tls1_2 -> CrtTlsVersion.TLS_V1_2
    SdkTlsVersion.Tls1_3 -> CrtTlsVersion.TLS_V1_3
}

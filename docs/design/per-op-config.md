...
The `withConfig` method is _more efficient_ than creating multiple clients from scratch (either directly from your
own `Config` or via `fromEnvironment`) if you are using runtime-managed `Closeable` resources such as the default HTTP
engine. Instances which use these defaults are tracked across clients as they are created, and the
underlying `Closeable` is only fully closed when the last remaining client using it is closed.
...
interface ServiceClient : SdkClient {
    ...
    // alt2.1: Trailing lambda builder
...
* Neither really play into to the adopted builder-based overloads that the SDK supports. Mixed usage of the two may
  adversely impact an author's ability to produce readable and maintainable code.
